...
/**
 * Task 14: the real, Redis-backed {@link RoomLeaseStore}.
 *
 * <p><b>NOT verified against a real Redis instance.</b> This development environment has no
 * Redis server and no working Docker daemon (same constraint recorded against
 ...
 * staging/production until it has actually run against a real Redis Cluster.
 *
 ...
 *
 * <p>{@link #tryAcquire} is three Redis round trips on the conflict path (SET NX, then GET +
 * GET), not one atomic script -- there is a narrow window between the failed SET NX and the
 * two GETs where the value could change again (another pod's lease expiring and a third pod
 * grabbing it, mid-read). Worst case this reads back a momentarily stale answer, corrected on
 * the very next {@link #tryAcquire} for that room; it cannot produce two pods both believing
 * they hold the same room, because that guarantee comes entirely from the atomicity of the
 * `SET NX` step itself. A Lua script would close this window too -- documented follow-up, not
 * done here since it cannot be verified without a real Redis.
 */
public final class RedisRoomLeaseStore implements RoomLeaseStore {

    /**
     * Atomic compare-and-extend: only the pod recorded as the current holder can renew its own
     * TTL. Needs a script because "extend TTL iff value matches" has no single Redis command.
     */
    ...
    public RedisRoomLeaseStore(RedisAsyncCommands<String, String> commands) {
