...
/**
 * Task 14: the narrow slice of Redis {@link RedisLeaseRoomOwnership} needs, kept as an
 * interface so the ownership/caching/fallback logic is testable without a real Redis
 * connection (the production implementation, {@code RedisRoomLeaseStore}, lives in
 * {@code engine.persistence} and is not exercised by any test in this repo -- there is no
 * Redis available in this development environment to verify it against, the same caveat
 ...
 * hop away from the Netty EventLoop (via {@link RedisLeaseRoomOwnership#ensureAcquired}),
...
public interface RoomLeaseStore {
     ...
     * (see {@link RedisLeaseRoomOwnership#renewAll()}) must treat {@code false}, and any
