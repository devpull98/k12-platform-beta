...
/**
 * Task 21 (2026-09-09): proves {@code EnginePodDiscovery} closes the Gateway-side gap found while
 ...
 * absent from {@code uni.gateway.engine.pods} when Gateway booted -- used to be unreachable no
 * matter what ownership algorithm Engine ran, because Gateway never dialed it.
 *
 * <p>Root cause, unchanged by Task 21 (confirmed by reading the code, not guessed): {@code
 * GatewayNetworkLifecycle.run()} still reads {@code uni.gateway.engine.pods} exactly ONCE at
 * Spring Boot startup and calls {@code FrameChannelClient.connect(...)} synchronously for each
 * entry -- that static list never grows after boot. What Task 21 adds instead, when {@code
 * uni.gateway.engine.pod-discovery.enabled}, is a second, independent path: {@code
 * EnginePodDiscovery} polls {@code ValkeyEnginePodResolver} (Engine pods self-announce into
 * room-store via {@code EnginePodPresence}) and dials anything not already connected.
 *
 ...
 * test: thêm pod thứ 3 giữa lúc 2 pod kia đang chạy phòng thật"). Discovery reaches it purely
 * through the room-store registry, never through {@code ENGINE_PODS}.
 *
 * <p>Run it with:
 * <pre>
...
class DockerComposeScaleUpIT {
    ...
    void should_routeNewRoomsToAFreshlyStartedEnginePod_withoutRestartingGateway() throws Exception {
        ...
        // Give EnginePodDiscovery a few poll cycles (GATEWAY_ENGINE_POD_DISCOVERY_POLL_INTERVAL_
        // SECONDS=3 in docker-compose.dev.yml) to pick engine-2 up from the room-store registry
        // before the join-retry loop below starts asserting.
