...
class DockerComposeResyncIT {
    ...
    void should_routeAcrossPodsAndSurviveResync_againstTheRealDockerStack() throws Exception {
        ...
        String joinTokenB = CODEC.sign(
                new JoinTokenClaims("student-docker-b", "room-docker-b", "session-docker-b", List.of("student")),
                Duration.ofMinutes(5));
        ...
        clientA.joinRoomWithRetry(joinTokenA, "Alice", 5, 2_000);
        clientB.joinRoomWithRetry(joinTokenB, "Bob", 5, 2_000);
        ...
        String reconnectJoinTokenA = CODEC.sign(
                new JoinTokenClaims("student-docker-a", "room-docker-a", "session-docker-a", List.of("student")),
                Duration.ofMinutes(5));
        clientA.simulateDisconnectAndReconnect("room-docker-a", reconnectJoinTokenA, "Alice");
