...
progress: "T1, T2, T4, T5, T10 xong. T6 MOT PHAN xong (GatewayPipeline + WS handshake +
  ...
  2026-09-08 (tiep, rieng phien nay): dat cau hoi 'LeaseBasedRoomOwnership co thay duoc Cluster
  Sharding khong'. Ra lai Task 14 bang cach doc code that (khong doan). Xac nhan: phan Engine (N
  khong con y nghia voi ownership) DA XONG + co chaos test that (DockerComposeChaosIT). Nhung tim
  ra 1 gap MOI, doc lap voi Task 14: GatewayNetworkLifecycle doc uni.gateway.engine.pods dung 1
  lan luc boot, FrameChannelClient khong co duong nao them pod moi sau do - 1 Engine pod hoan toan
  moi khong bao gio nhan duoc goi tin nao, du LeaseBasedRoomOwnership dung 100%. Mo Task 21 (moi)
  cho gap nay. Viet san 1 test do co chu y (DockerComposeScaleUpIT, @Disabled, khong chay trong
  mvn test mac dinh de khong pha build) de tai lieu hoa gap bang test that thay vi chi ghi chu -
  se tu xanh khi Task 21 xong, khong sua assertion. Them service engine-2 vao
  docker-compose.dev.yml (co y KHONG co trong ENGINE_PODS cua gateway/gateway-1) chi de phuc vu
  test nay. Cap nhat system-architecture.md (SS9.2 Rui ro 4, SS7.6, ADR-002 note) + CLAUDE.md
  (tach 'Cluster Sharding' thanh 2 van de rieng: pod crash da xong, them pod moi giua phien van
  chua). Khong doi ket luan cu ve Task 19 (runbook cam auto-scale) - van giu nguyen hieu luc, gio
  can CA Task 14 (bat production + verify staging that) LAN Task 21 xong moi duoc noi long. mvn
  clean install toan reactor: BUILD SUCCESS (khong Docker), khong doi hanh vi runtime nao, chi
  them 1 test @Disabled + 1 service compose moi.
  2026-09-08 (tiep, cung phien): dong Task 17 that (B4) - nguoi dung hoi missed_step_policy nghia
  la gi roi bao noi luon. Nhan ra khong can cho luong late-join day du (§4.8, van chua ton tai) -
  chi can biet 'bao nhieu cau hoi da bat dau luc join' la du de ap dung ZERO co chu dich.
  RoomState them questionsStartedCount (tang moi lan startQuestion, song sot qua restore - Hot
  Snapshot them field moi, khong doi SnapshotEnvelope.SCHEMA_VERSION vi dinh dang noi bo khong co
  version rieng va chua co du lieu production nao) + PlayerRecord.missedStepsAtJoin (final, chup
  dung 1 lan luc computeIfAbsent tao record moi - reconnect khong bao gio chay lai). Them lop
  fail-fast thu 3 (applyMissedStepPolicy) sau DefinitionLoader + RoomActor.create. Them
  missedStepsFor(studentId) package-private lam tin hieu quan sat duoc cho test. Tat ca overload
  moi (RoomState constructor + restore) la additive, khong pha call site cu nao (RoomActorTest,
  RoomStateSnapshotTest, RoomSupervisorTest, RoomActorSnapshotTest deu khong doi). Test moi
  RoomStateMissedStepPolicyTest (8 case). Prove-it: tam comment questionsStartedCount++, xac nhan
  dung 6/8 test Red truoc khi tra lai Green. mvn clean install toan reactor: BUILD SUCCESS, 120
  test uni-game-engine (112 cu + 8 moi), khong leak. Chi tiet day du: plan.md Task 17.
  2026-09-08 (tiep, cung phien): dong not AC cuoi cung con lai cua Task 14 (do thoi gian phuc hoi
  that) - nguoi dung bao 'bat dau Task 14 luon di'. Docker daemon dang chay that trong phien nay
  (khac gia dinh truoc). Build jar that (mvn clean package -DskipTests), dung docker-compose.dev.yml
  that (room-store + kafka + engine-0/1 + gateway/gateway-1), chay DockerComposeChaosIT 6 lan voi
  RUN_DOCKER_IT=true. Ket qua: 5/6 xanh, thoi gian tu luc docker compose kill pod dang giu phong
  toi luc phong phuc hoi dung roster tren pod khac: 21834/24903/24843/24905/27917 ms (21.8s-27.9s,
  trung binh ~24.9s) - khop ly thuyet (du TTL 0-20s + chu ky renewAll moi ttl/3~6.67s). 1/6 lan
  fail thoang qua, khong lap lai - khop canh bao co san trong comment cua test ('a tighter window
  flaked once'), khong phai regression. Them log do thoi gian that vao DockerComposeChaosIT
  (recoveryMs) de lan do sau (staging that) co ngay so lieu. Phat hien quan trong khi doc lai
  system-architecture.md: con so '10-50ms' o SS6.5/ADR-003 KHONG PHAI tong thoi gian phuc hoi cua
  Task 14 - do la T_load (buoc doc snapshot tu Valkey, dung chung ca lease GD1 lan Cluster Sharding
  GD2 tuong lai), nham lan cu da co tu truoc. Da sua SS6.5 (dinh chinh) + ADR-002 GD1-note (them so
  do that) + plan.md Task 14 (tick AC cuoi, doi header sang XONG) + _context.md B1 (them so do).
