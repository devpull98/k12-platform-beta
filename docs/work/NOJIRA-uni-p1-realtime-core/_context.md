...
> `uni.engine.redis.enabled` vẫn `false` trong `application.yml`, chỉ bật qua env var ở
...
> `docker-compose.dev.yml` thật, `RedisRoomLeaseStore`/`RedisSnapshotStore` đã ghi/đọc Redis thật
