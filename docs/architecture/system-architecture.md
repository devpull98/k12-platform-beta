...
4. **Hot path ngắn nhất**: Không có Message Broker, Redis hay Database chen ngang giữa client và RoomActor.
...
> [!NOTE]
> **Hạ tầng hiện tại:** Redis Cluster và Kafka **đã sẵn sàng và được tích hợp ngay từ đầu** ở tầng Async Backplane (Redis cho Ticket `SETNX` lúc handshake và Hot Snapshot < 5 KB; Kafka cho Event Log trận đấu). PostgreSQL lưu trữ kết quả thi đấu sau phiên. Toàn bộ Hot Path tiếp tục duy trì 100% qua TCP nội bộ Netty thuần, tuyệt đối không đi qua Redis hay Kafka.

### 1.6 Ba thứ cố ý không có
- **Redis Pub/Sub trên hot path**: Không dùng — mỗi phòng có đúng 1 RoomActor chủ sở hữu, không có shared state để phải đồng bộ qua backplane.
...
|---|---|---|---|
| **Gateway** | Không *(trừ RouteCache tự lành)* | Handshake, ticket auth (`SETNX` Redis), rate limit, định tuyến học được, fan-out zero-copy | Chấm điểm, đọc luật chơi, gọi DB, giữ state phòng |
| **Engine** | **Có** (in-memory) | Luật chơi, chấm điểm, FSM, tick coalescing, lưu Hot Snapshot async sang Redis, phát event sang Kafka | Chạm socket client trực tiếp, blocking I/O trên dispatcher actor |
| **Redis Cluster** | Có (Đã sẵn sàng) | Hot Snapshot (< 5 KB), fencing epoch, chặn replay ticket (`SETNX`), session registry | **Nằm trên đường đi của gói tin hot path (Cấm Redis Pub/Sub)** |
...
| **Netty EventLoop** | **Platform thread cố định (cores × 2)** | Hoàn toàn non-blocking. **Cấm tuyệt đối DB, Redis, HTTP, JSON parse hoặc blocking I/O** |
| **I/O ngoại vi (DB, Redis)** | **Virtual thread** | Thích hợp cho blocking I/O; dùng `ReentrantLock`, cấm `synchronized` tránh thread pinning |
...
3. Gateway: verify ticket 1 lần lúc handshake:
            ...
            b. Chặn replay trên Redis Cluster: SET ticket:{jti} "1" EX 30 NX
...
- **Cơ chế chống Replay đã chốt:** Sử dụng Redis Cluster `SET ticket:{jti} "1" EX 30 NX` giải quyết triệt để việc cưỡng chế vé 1 lần giữa nhiều Gateway pod mà không làm chậm hot path. Thuật toán ký và phân phối secret được nạp lúc boot.
...
Cơ chế này đạt được 3 tính chất: **Không cần Redis trên hot path** · **Không cần TTL** · **Không bao giờ lệch sự thật**.
...
t ≈ 20s     Shard tái phân bổ sang node khác: Actor mới khởi tạo, tăng epoch, nạp snapshot từ Redis
            (áp dụng stagger jitter 0–500ms nạp snapshot để chống bão request lên Redis).
...
- **Định tuyến tự học** qua `owner_pod_id` (không cần Redis/TTL trên hot path).
- **Tích hợp Redis Cluster**: Chặn replay ticket (`SET ticket:{jti} 1 EX 30 NX` lúc handshake), lưu Hot Snapshot (< 5 KB) định kỳ + Fencing epoch bất đồng bộ.
...
**Hệ quả:** **Virtual threads không được bật** ở cả hai service · **không DB/Redis/HTTP call nào
...
1. Đã là tiêu chuẩn thực tế trong toàn bộ hệ sinh thái dữ liệu & networking lớn của Java (Kafka, Cassandra, Netty, Trino, Elasticsearch) và Redis.
...
### 9.3 Rủi ro Tầng Hạ Tầng Phụ Trợ Redis Cluster & Kafka
